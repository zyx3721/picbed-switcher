...
# picbed-switcher 二进制部署在线更新脚本：分页抓取 GitHub 全部 Release，选择版本后自动完成下载校验、停止服务、替换产物并重启
# 前端为 Vite 静态产物，由 Nginx 托管，更新时仅替换静态目录，无需启停前端进程

REPO="zyx3721/picbed-switcher"
INSTALL_DIR="${INSTALL_DIR:-/data/picbed-switcher}"
...
if [ -z "${BACKEND_SCRIPT:-}" ]; then
    if [ -f "$INSTALL_DIR/run.sh" ]; then
        BACKEND_SCRIPT="$INSTALL_DIR/run.sh"
    else
        BACKEND_SCRIPT="$SCRIPT_DIR/run.sh"
    fi
fi
...
cleanup() {
    case "$TMP_DIR" in
        /tmp/picbed-switcher-update.*) [ -d "$TMP_DIR" ] && rm -rf "$TMP_DIR" ;;
    esac
}
trap cleanup EXIT

# check_env 检查安装目录、后端启停脚本与基础依赖是否就绪
check_env() {
...
show_version_list() {
    ...
    if [ -n "$CURRENT_VERSION" ]; then
        echo_log_info "当前系统 picbed-switcher 版本为 \033[32m$CURRENT_VERSION\033[0m ，仓库 $REPO 可安装的版本有（共 $total 个）：\n"
    else
        echo_log_warn "未检测到已安装的 picbed-switcher 版本，仓库 $REPO 可安装的版本有（共 $total 个）：\n"
    fi
...
confirm_install() {
    ...
    while true; do
        read -rp "即将${action} picbed-switcher ${TARGET_TAG}${tip}，是否继续? [y/N]: " yn
        case "$yn" in
            [Yy]*) break ;;
            [Nn]*|"")
...
prepare_assets() {
    ...
    BACKEND_ASSET="picbed-switcher_${ver}_linux_${ARCH}.tar.gz"
    FRONTEND_ASSET="picbed-switcher-frontend_${ver}.tar.gz"
...
download_packages() {
    echo ""
    echo_log_info "\033[32m开始下载 picbed-switcher ${TARGET_TAG} 安装包...\033[0m"
...
extract_packages() {
    ...
    [ -f "$TMP_DIR/backend/linux_${ARCH}/picbed-switcher" ] || echo_log_error "后端安装包内未找到 picbed-switcher 二进制文件"
    [ -f "$TMP_DIR/frontend/index.html" ] || echo_log_error "前端安装包内未找到 index.html"
}

# stop_services 通过启停脚本停止后端服务
stop_services() {
    echo ""
    echo_log_info "停止后端服务"
    "$BACKEND_SCRIPT" stop
}

# install_packages 备份并原子替换后端二进制与前端静态产物目录，保留原有属主权限
install_packages() {
    echo ""
...
main() {
    ...
    detect_arch
    get_current_version
    TMP_DIR=$(mktemp -d /tmp/picbed-switcher-update.XXXXXX)
    fetch_all_releases
    show_version_list
    select_version
    confirm_install
