...
图床转站助手是一款专为解决Markdown文档中图床地址批量转换需求而开发的工具平台。支持GitHub、Gitee、腾讯云COS、阿里云OSS、七牛云等5种主流图床的适配与切换，提供简洁易用的操作界面，保障用户数据安全。
...
- `GET /api/picbed/configs/:id` - 获取单个配置
...
- `POST /api/convert/upload` - 上传Markdown文档
- `POST /api/convert/process` - 执行转换
- `GET /api/convert/download/:id` - 下载转换后的文档
